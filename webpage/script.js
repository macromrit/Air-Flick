...
async function uploadImage() {
    ...
    try {
        ...
        formData.append('file', selectedImageFile);
            ...
            await sleep(1000); // Pause for 1/2 seconds (500 milliseconds)
...
// // grab image from peer
async function fetchAndShowPeerImage() {
    try {
        const response = await fetch(`${apiBaseUrl}/get-nearest-peer`);
        
        if (!response.ok) throw new Error('No peer found');

        const data = await response.json();
        const peerIp = data['peer-ip'];

        // resolving caching issues
        const imageUrl = `http://${peerIp}:8000/get-image?cacheBust=${Date.now()}`;

        // Show image in popover
        const popover = document.getElementById('peer-popover');
        const image = document.getElementById('peer-image');
        image.src = imageUrl;

        popover.classList.remove('hidden');
    } catch (err) {
        console.error('Error fetching image from peer:', err);
        showNotification('Peer not Found or Invalid Image URL.');
    }
}
